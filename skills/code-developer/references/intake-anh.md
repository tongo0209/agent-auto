...
4. **Quy tắc nhận ảnh** (subagent chỉ đọc được ảnh là FILE trên đĩa, qua đường dẫn):
   ...
   - **Link `figma.com` trong design/brief (máy này)**: có skill riêng `/figma-cut` (`~/.claude/skills/figma-cut`) bóc qua Figma REST API thành asset đã verify + `coords.json` — không cần Photoshop/Figma desktop. Task có link Figma mà chưa có `_auto-export/figma-*/assets/` → chạy `/figma-cut <KEY|url>` TRƯỚC rồi mới giao việc; asset + toạ độ lấy từ `coords.json` (x,y,w,h theo góc frame, dùng thẳng cho px tuyệt đối R-CDN-4); text vùng data-bind lấy từ `textLive`, ĐỪNG cắt chữ thành ảnh.
