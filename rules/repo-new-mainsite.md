...
| **R-TWIG-4** | MUST | KHÔNG `git commit` / `git push` ở repo này (repo phụ — user tự review diff & đẩy). `git push` có hook `G-GIT-2` chặn thật; `git commit` thì KHÔNG hook nào chặn (14/8/2026 user gỡ cổng) — đây là luật tự giác, đừng vì lệnh chạy được mà commit. |
